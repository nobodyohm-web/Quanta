...
> **Version** : v3.16.0 (crypto-only, hard-fork **v10 « ce qui est signé est sans ambiguïté »** — remédiation de l'audit externe du 13/08/2026) | **Stack** : Rust (Tauri 2.0) + Svelte 5 | **Coin** : QUANTA
> **Licence** : Apache-2.0
> **Status** : ✅ P2P vérifié entre **deux daemons réels** sur la DHT publique (01/08/2026 — découverte,
> `Hello` dispatché, sync `ChainSegment`, convergence, partage v9 observé) · ⚠️ **deux machines
> physiques derrière deux NAT : jamais éprouvé sur le code actuel** (le jalon du 06/05/2026 précède
> PQ-ENVELOPE-1 de deux mois, et la régression GOSSIP-MTU-1 l'a invalidé sans que rien ne le signale)
> · ⚠️ alpha, non audité par un tiers
...
└─ **OPEN-DOOR-1** = 1 bloc sur 16 (`OPEN_SLOT_EVERY_BLOCKS`) est un **slot ouvert**
    proposable par n'importe quelle adresse, bondée ou non — l'unique chemin d'un
    nouvel arrivant vers sa première pièce
```
...
- **NET-5** : `TORUS_PROTOCOL_VERSION = 10` (2→3 PQ-MIG-5 : genèse PQ ; 3→4 LIVE-3B : slashes-unbonding ; 4→5 hard-fork v4 : genèse propre + PROPOSER-1 + enveloppes PQ ; 5→6 **MSIG-1** : multisig ML-DSA natif — additif, tx mono-clé byte-identiques, genèse inchangée ; 6→7 **remédiation d'audit** : C2 expéditeur synthétique confiné, C3 unstake borné par l'enjeu bondé, H2 époque de vote bornée, H1/H3 identifiant d'enveloppe canonique + dedup post-signature ; 7→8 **MINT-EXACT-1 + OPEN-DOOR-1** : récompense de bloc recalculée au lieu d'être bornée, slot ouvert tous les 16 blocs ; 8→9 **REWARD-SHARE-1** : une coinbase par bénéficiaire, répartition recalculée par chaque nœud ; 9→10 **CANON-1 + NONCE-ONCHAIN-1 + FORK-RANK-1** (audit externe 13/08/2026) : préimage de tx, feuille Merkle et en-tête de bloc passent à un encodage **injectif** (domaine + champs préfixés en longueur — le modèle que `sm/finality_vote.rs` appliquait déjà correctement), et l'unicité d'une transaction devient une **règle de consensus** (nonce de compte séquentiel vérifié à l'inclusion). Le **départage de fork** cesse d'être « le plus grand hash gagne » — un critère qu'un attaquant sans enjeu gagnait pour le prix d'un broyage — et passe par le **rang d'élection pondéré par l'enjeu** (`election_ranking`, tirages sans remise sur la graine de `elect_leader`) ; la profondeur de réorganisation est bornée à 128 blocs indépendamment de la finalité (long-range), `energy_kwh` entre dans le hash d'en-tête (M-09) et le vestige `tx.signature` en sort (MOY-6). Toute signature et tout hash changent, genèse comprise) ; peers incompatibles loggués
...
- **MINT-EXACT-1** : la récompense d'un bloc = `emission_for_block(offre minée avant ce
  bloc)`, **fonction pure de la chaîne**. Le producteur la frappe (`mint_block_reward`),
  chaque récepteur la **recalcule**. Avant, le montant venait d'un calcul local (part
  Shapley sur des watts auto-déclarés) et le réseau ne pouvait que le borner à
  `64 × emission_for_tick` — soit `32 × N` fois le montant honnête
- **REWARD-SHARE-1** : cette récompense se **partage** — moitié au producteur, moitié
  à parts égales entre les **participants récents** (adresses distinctes ayant produit
  un bloc dans les `SHARE_WINDOW_BLOCKS` = 32 derniers, une participation que la chaîne
  prouve déjà via `block.miner`). La répartition est recalculée par chaque nœud
  (`validate_block_reward_plan`), donc imposée et non suggérée ; elle est **invariante
  d'échelle** (émettre moins est permis, rogner la part des autres non). Aucun champ
  wire nouveau
...
```
513 tests lib + 1 intégration, 0 failures
...
├── security_tests (44 tests) — signatures, replay, nonce, rate limit
...
| 2026-08-01 | **🤝 v3.15.0 — hard-fork v9 « la récompense se partage » (`TORUS_PROTOCOL_VERSION` 8→9).** On n'était payé qu'en **scellant** un bloc : la quasi-totalité des participants ne touchait jamais rien, et les votes de finalité — pourtant coûteux (diffusion ML-DSA de 3,9 Ko) — ne rapportaient rien du tout. **REWARD-SHARE-1** : la récompense canonique se partage désormais entre le producteur (moitié, barème `PROPOSER_SHARE_NUM/DEN`) et les **participants récents** — les adresses distinctes ayant produit un bloc dans les `SHARE_WINDOW_BLOCKS` = 32 précédents, à parts égales, le reste de division entière revenant au producteur (conservation exacte au µQTA). **Aucun champ wire nouveau** : la participation est déjà prouvée par la chaîne (`block.miner`, lié au hash par BLK-HASH-1), donc il n'a pas fallu embarquer les votes dans les blocs. Le partage est **imposé, pas suggéré** — chaque nœud recalcule le plan (`validate_block_reward_plan`, source unique produire/vérifier comme `expected_slash_consumption`) et rejette un producteur qui capte tout ou paie un tiers hors plan ; le plan est **invariant d'échelle**, donc émettre moins reste permis sans pouvoir rogner la part des autres. Récompenser la production récente plutôt que l'enjeu bondé en fait une prime de **liveness** : un validateur hors ligne sort de la fenêtre et cesse d'être payé, là où pondérer par l'enjeu aurait recréé une rente de capital que la doctrine refuse. Bug de production trouvé au passage et fermé : une frappe restée en attente après l'intégration d'un bloc distant devenait **périmée** (calculée pour une autre hauteur, donc un autre ensemble de participants) et aurait fait rejeter le bloc suivant par tout le réseau — `mint_block_reward` révoque désormais toute récompense en attente avant d'en frapper une neuve. **507 tests + 1 intégration** (dont 5 neufs : participants dérivés de la chaîne et fenêtre glissante, partage conservatif au µQTA, producteur gourmand rejeté, bénéficiaire hors plan rejeté, invariance d'échelle, frappe périmée révoquée), clippy `--all-targets -D warnings` propre |
| 2026-08-01 | **💰 v3.14.0 — hard-fork v8 « la monnaie exacte, la porte ouverte » (`TORUS_PROTOCOL_VERSION` 7→8).** Deux règles de consensus changent, toutes deux issues d'une recherche d'état de l'art recoupée avec le code. **MINT-EXACT-1** : le montant de la récompense était calculé **localement** par le sceleur (part Shapley dérivée de watts **auto-déclarés** par les pairs, donc invérifiables) et le réseau ne pouvait que le **borner** à `64 × emission_for_tick` ; le montant honnête valant `2 × emission_for_tick / N`, la marge exploitable était de **`32 × N`** — à 100 nœuds, tout validateur bondé pouvait se frapper **3 200 fois** sa récompense légitime, à chaque bloc, accepté par tous (seul le plafond dur des 100 M était vérifié). Désormais la récompense est `emission_for_block(offre minée avant ce bloc)`, **fonction pure de la chaîne** : le producteur la frappe (`mint_block_reward`), chaque récepteur la **recalcule**. Un seul changement en ferme six : l'inflation, le **fork privé silencieux** (`mine_tx` scellait tout seul à 10 tx en attente, sans contrôle de proposeur et **sans jamais diffuser** — un nœud non éligible s'affichait un solde que le réseau ignorait), le **solde fantôme**, le trafic mort (tx `Mining` diffusées puis rejetées par MINT-GUARD-1), l'**énergie auto-déclarée sur le chemin monétaire**, et l'**effondrement de l'émission en `1/N`** (les parts Shapley sommant à 1, le réseau créait *moins* à mesure qu'il grandissait). `mine_tx` devient `#[cfg(test)]` : en release **rien** ne peut poser de `Mining` au mempool. **OPEN-DOOR-1** : `PROPOSER-1` refermait le réseau **définitivement** au premier staker — sans faucet, airdrop ni premine, un nouvel arrivant avait besoin d'un enjeu pour proposer et de proposer pour gagner. Un bloc sur 16 devient un **slot ouvert** proposable par n'importe quelle adresse. Le prix est nommé et borné : le **trilemme de vulnérabilité Sybil** (Platt, Platt & McBurney, 2024) démontre qu'on ne peut pas être à la fois sans-permission, résistant aux Sybils et gratuit — on relâche « gratuit » d'exactement un seizième, et une ferme d'identités ne peut jamais capter davantage. **502 tests + 1 intégration** (dont 7 nouveaux : borne exacte à ±1 µQTA, bloc de production valide par construction, double-frappe rejetée, admission qui ne scelle plus, cadence pure de la hauteur, nouvel arrivant admis sur slot ouvert et refusé hors slot), clippy `--all-targets -D warnings` propre |
...
| 2026-08-01 | **📡 Le nœud était MUET sur tout réseau réel depuis le fork v4 (GOSSIP-MTU-1) — trouvé en lançant deux vrais daemons, pas par un test.** `iroh-gossip` plafonne un message à **4 096 o** (`DEFAULT_MAX_MESSAGE_SIZE`) ; depuis PQ-ENVELOPE-1 une enveloppe porte une signature ML-DSA-65 (3 309 o) **et** la clé publique de l'émetteur (1 952 o), en hexadécimal dans du JSON — un simple `Hello` pèse **~15 Ko, presque 4× le plafond**. Deux nœuds se découvraient par la DHT, établissaient QUIC, se déclaraient voisins (`NeighborUp`)… et **n'échangeaient rien** : ni Hello, ni bloc, ni tx, ni vote de finalité. L'émission échouée était **comptée comme réussie** dans `stats.messages_sent`, donc aucune métrique ne contredisait le silence. Correctif : `Gossip::builder().max_message_size(MAX_RAW_ENVELOPE_BYTES)` — une seule limite, celle de l'étape ① du dispatcher, que le réseau ne peut plus contredire en silence. Deux bugs de la même session : **PEER-SELF-1** (le nœud s'inscrivait lui-même dans `peer_info`, dont la taille *est* le compteur de pairs — un nœud parfaitement seul affichait « 1 pair », **et** la cadence adaptative de RDV-1, qui ne réessaie vite que tant que zéro pair n'est connu, ne se déclenchait jamais : une fenêtre de découverte manquée devenait 25 min de panne) et **HELLO-NEIGHBOR-1** (`NeighborUp` ne déclenchait aucun Hello → jusqu'à 120 s d'invisibilité mutuelle après connexion, donc aucune comparaison de hauteur, donc aucun sync). Vérifié en vivant après correctif : découverte mutuelle ~9 s, **premier `Hello` jamais dispatché entre deux processus Quanta depuis v4**, sync `ChainSegment` (blocs 1→4), convergence, et un bloc #5 portant **deux coinbases** de 1 999 999 µQTA sommant exactement à la récompense canonique (REWARD-SHARE-1 en vivant, un nœud payé sans avoir rien scellé). **Leçon devenue règle** : la suite était verte de bout en bout — 507 tests dont un test d'intégration deux-nœuds *in-process* — et le jalon « P2P vérifié entre 2 machines » datait de **deux mois avant** la régression. Un test in-process ne peut pas voir un bug qui n'existe qu'entre deux processus OS ; avant toute affirmation P2P, lancer deux vrais daemons et vérifier **par RPC**, jamais par les logs. **513 tests + 1 intégration**, clippy propre |
| 2026-08-01 | **🪪 v3.15.1 — une identité, une source (issue #32).** `getinfo` publiait un champ `node_id` qui **n'était pas l'identité du nœud** : `WillowNode::new()` le posait à `BLAKE3(Uuid::new_v4())`, donc une valeur neuve à chaque processus, changeant à chaque redémarrage et ne correspondant à **rien d'observable** sur le réseau. Il avait exactement la forme d'un EndpointId (64 hex), et c'est ce qui l'a rendu invisible : tout ce qui s'y indexait — tableau de bord d'opérateur, moniteur de dépôts, inventaire de pairs — voyait un « autre nœud » après chaque relance, sans erreur. Trouvé en écrivant `docs/ops/two-machines.sh` : sa vérification de persistance, écrite contre ce champ, annonçait « RDV-0 est cassé » alors que `node_key` était intact sur le disque. Le champ aléatoire est **supprimé** (`WillowNode::node_id` et `NodeStatus::node_id`) — il n'avait qu'un seul lecteur, le `getinfo` fautif ; le front lisait déjà `peer_id`. `getinfo.node_id` rapporte désormais l'**EndpointId Iroh**, dérivé du `node_key` persisté (RDV-0), donc stable au redémarrage et identique à la chaîne qu'un pair compose — vide tant que l'endpoint n'est pas lié, ce qui est plus honnête qu'une valeur plausible indiscernable d'une vraie. **Aucun changement de protocole** (v9 inchangé) : c'est un défaut de *rapport*, pas de règle. **513 tests + 1 intégration** (dont 5 neufs : identité persistée au redémarrage, data-dir séparé = nœud séparé, clé corrompue régénérée puis stable, aucune identité annoncée avant liaison de l'endpoint, contrat `getinfo.node_id`), clippy `--all-targets -D warnings` propre |
| 2026-08-02 | **🤝 `docs/ops/peer.sh` — un second opérateur peut rejoindre le réseau sans avoir lu le code.** Un seul script (`start` / `status` / `logs` / `update` / `report` / `stop`) et **Rust seul** : vérifié sur un clone neuf, `quanta-node` compile sans Node, sans npm et sans toolchain Tauri. `status` tient en un écran les trois choses qui décident — `peers`, la ligne `TIP@<hauteur> = <hash>` à comparer entre machines, et le couple `build` + `protocole` — parce que les deux pannes silencieuses du projet sont un binaire périmé et **deux pairs sur des versions de protocole différentes, qui s'ignorent exprès et donnent exactement l'image d'une panne de réseau**. `update` est un `git pull` **auditable** : il affiche les commits entrants **avant** de les appliquer, refuse d'écraser des modifications locales, n'accepte qu'un fast-forward, recompile et relance — et prévient si `TORUS_PROTOCOL_VERSION` a changé. **Pas de canal de commande à distance, par principe** : un nœud de monnaie à qui l'on pourrait dicter le code qu'il exécute est une porte dérobée. `report` produit un diagnostic partageable, mot de passe de coffre et jeton RPC **retirés** (les IP restent, et c'est dit). **Bug trouvé en s'en servant** : `two-machines.sh` comparait le hash de la **dernière transaction** du bloc de tête au lieu du hash **du bloc** — le RPC sérialise les clés JSON par ordre alphabétique, donc `hash` précède `transactions`, et `tail -1` attrapait la dernière tx ; la ligne de convergence, seule preuve du test à deux machines, n'attestait pas ce qu'elle annonçait. Corrigé dans les deux scripts. Journalisation ciblée au passage (iroh loggue un `poll_send` par paquet UDP : **1 793 lignes dont 57 de Quanta → 35 dont 16**, un rapport devenait illisible). `SECURITY.md` corrigé : « le réseau n'a jamais tourné au-delà de deux machines physiques » laissait entendre que ce jalon valait pour le code actuel. Modèles d'issue GitHub ajoutés. **Vérifié en vivant** : deux nœuds lancés par le script se découvrent, convergent (`TIP@3` identique des deux côtés), deux adresses distinctes payées (REWARD-SHARE-1), et l'identité réseau est intacte après un `stop`/`start`. Aucun changement de protocole (v9), aucun code Rust touché |

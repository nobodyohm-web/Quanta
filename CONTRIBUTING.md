...
5. **No `unwrap()`** on any path that can fail. `Result<T, E>` and `?`. The tree currently holds
   one `unsafe` block; adding a second requires a written justification in the PR.

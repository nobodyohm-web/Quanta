...
> - **Review baseline:** commit `97123d3a4ac6cfae2f2fd76456d1bc173027b4fa` (v3.10.0); we will
>   freeze a dedicated `audit-baseline` tag at engagement start.
> - **Package:** `docs/audit/README.md`, `THREAT-MODEL.md`, `SCOPE.md`.
> - **Suggested scope:** P0 = consensus core `sm/` (8_492 LOC), ledger `p2p/ledger/`
>   (6_369 LOC), cryptography `security/` (2_142 LOC); P1 = network pipeline (6_315 LOC).
>   Line counts include inline `#[cfg(test)]` tests. Engagement shapes (crypto / consensus /
>   network) are described in `SCOPE.md`.
> - **Verification:** `cargo test` (449 library + 1 integration test, 0 failures at baseline,
...
- **Testable claims.** 449 library + 1 integration test at baseline, a deterministic
